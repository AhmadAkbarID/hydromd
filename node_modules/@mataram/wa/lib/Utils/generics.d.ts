...
import type { MataramEventEmitter, MataramEventMap, ConnectionState, WACallUpdateType, WAMessageKey, WAVersion } from '../Types/index.js';
...
export declare function bindWaitForEvent<T extends keyof MataramEventMap>(ev: MataramEventEmitter, event: T): (check: (u: MataramEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
export declare const bindWaitForConnectionUpdate: (ev: MataramEventEmitter) => (check: (u: Partial<ConnectionState>) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
/**
 * utility that fetches latest WA Web version from the repo.
 * Use to ensure your WA connection is always on the latest version
 */
export declare const fetchLatestVersion: (options?: RequestInit) => Promise<{
    version: WAVersion;
    isLatest: boolean;
    error?: undefined;
} | {
