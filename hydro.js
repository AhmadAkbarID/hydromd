...
└────────────────────────────────────────┘
        ...
        switch (command) {
            ...
            case 'menu': {
                ...
                if (args[0] === 'cpanel') return reply(global.cpanelmenu(prefix));
                ...
                try {
                    if (typeof supabase !== 'undefined') {
                        let { data } = await supabase.rpc('get_rating_stats');
                        if (data?.total) {
                            rata2 = String(data.rata2);
                            totalRating = data.total;
                        }
                    }
                } catch (e) { }
...
🚀 *Pᴏᴡᴇʀᴇᴅ Bʏ ${global.botname}*`)
            ...
            case 'chatexai': {
                ...
                break;
            case 'gemini': case 'geminiai': {
                if (!text && !m.quoted) return replyquery(mess.query.text);
                try {
                    let prompt = text || 'Jelaskan gambar ini.';
                    let imageBuffer = null, imageMime = null;
                    let quotedCtx = null;

                    if (m.quoted) {
                        const q = m.quoted;
                        const qSender = q.sender || q.key?.participant || q.key?.remoteJid || '';
                        const qNumber = qSender.split('@')[0];
                        let qName = qNumber;
                        try {
                            const nameResult = hydro.getName(qSender);
                            qName = (nameResult instanceof Promise) ? await nameResult : nameResult;
                        } catch { }

                        quotedCtx = {
                            senderNumber: qNumber,
                            senderName: qName || qNumber,
                            text: q.text || q.body || q.caption || '',
                            type: q.mtype?.replace('Message', '') || 'unknown',
                            isBot: qSender === hydro.decodeJid(hydro.user.id),
                        };

                        if (q.mimetype?.startsWith('image/')) {
                            imageBuffer = await q.download();
                            imageMime = q.mimetype;
                        }
                    }

                    const hasil = await askGemini(prompt, imageBuffer, imageMime, quotedCtx);
                    reply(hasil);
                } catch (e) {
...
`.trim();
            ...
            case 'rating': {
                ...
                if (!isNaN(nilai)) {
                    ...
                    let [e1, e2, e3] = await Promise.all([
                        supabase.from('ratings').select('id').eq('user_id', m.sender).single(),
                        supabase.from('ratings2').select('id').eq('user_id', m.sender).single(),
                        supabase.from('ratings3').select('id').eq('user_id', m.sender).single(),
                    ]);

                    let checkError = (!e1.data && !e2.data && !e3.data) ? e1.error : null;
                    let existing = e1.data || e2.data || e3.data;
                    ...
                    let { error } = await supabase.from('ratings').insert([{ user_id: m.sender, nilai }]);
                    if (error) {
                        let { error: e2err } = await supabase.from('ratings2').insert([{ user_id: m.sender, nilai }]);
                        if (e2err) {
                            let { error: e3err } = await supabase.from('ratings3').insert([{ user_id: m.sender, nilai }]);
                            if (e3err) {
                                await react('❌');
                                return replyfail(`⚠️ Gagal menyimpan rating: ${e3err.message}`);
                            }
                        }
                    }
            ...
            case 'cekrating': {
                ...
                let { data, error } = await supabase.rpc('get_rating_stats');
                ...
                await reply(`📊 Rata-rata rating bot ini adalah *${data.rata2}* ⭐\nDari total ${data.total} penilai.`);
                await react('✅');
            }
                break;
            case 'infobot': {
            ...
            case 'cekidgb':
            case 'cekidgroup': {
                if (!text) return replyquery(global.mess.query.link);

                const isChannel = text.includes('https://whatsapp.com/channel/');
                const grupRegex = /chat\.whatsapp\.com\/([A-Za-z0-9]+)/i;
                const boolStatus = (v) => (v === true ? 'Aktif' : v === false ? 'Tidak Aktif' : (v ?? '-'));

                if (!isChannel && !grupRegex.test(text)) return replyquery(global.mess.query.link);
                ...
                try {
                    if (isChannel) {
                        ...
                        if (!code) return replyquery(global.mess.query.link);
...
✅ *Verifikasi:* ${res.verification == 'VERIFIED' ? 'Terverifikasi' : 'Tidak'}`;
                                    ...
                                    interactiveMessage: proto.Message.InteractiveMessage.create({
                                        ...
                                        nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
                                            buttons: [
                                                {
                                                    name: 'cta_copy',
                                                    buttonParamsJson: JSON.stringify({ display_text: '📋 Salin ID', copy_code: channelId })
                                                },
                                                {
                                                    name: 'cta_copy',
                                                    buttonParamsJson: JSON.stringify({ display_text: '📋 Salin Nama', copy_code: channelName })
                                                }
                                            ]
                                        })
                                    })
                    ...
                    let pp = null;
                    try { pp = await hydro.profilePictureUrl(g.id, 'image'); } catch { }
...
📅 *Dibuat:* ${g.creation ? new Date(g.creation * 1000).toLocaleString('id-ID') : '-'}
👑 *Creator:* ${creator}
...
${admins.length ? admins.map(a => `- @${a.id.split('@')[0]} (${a.admin})`).join('\n') : '-'}`;
                    ...
                    const buttons = [
                        {
                            name: 'cta_copy',
                            buttonParamsJson: JSON.stringify({ display_text: '📋 Salin ID', copy_code: groupId })
                        },
                        {
                            name: 'cta_copy',
                            buttonParamsJson: JSON.stringify({ display_text: '📋 Salin Nama', copy_code: groupName !== '-' ? groupName : '' })
                        }
                    ];
                    ...
                    if (pp) {
                        ...
                        msgContent = generateWAMessageFromContent(m.chat, {
                            viewOnceMessage: {
                                message: {
                                    interactiveMessage: proto.Message.InteractiveMessage.create({
                                        body: proto.Message.InteractiveMessage.Body.create({ text: teks }),
                    ...
                    msgContent.message.viewOnceMessage.message.interactiveMessage.contextInfo = { mentionedJid: mentioned };

                    await react('✅');
                    return hydro.relayMessage(msgContent.key.remoteJid, msgContent.message, { messageId: msgContent.key.id });

                } catch (err) {
                    console.log('[CEKID ERROR]', err);
                    await react('❌');
                    return replyfail(global.mess.error.fitur);
                }
            }
